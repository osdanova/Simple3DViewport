...
namespace Simple3DViewport.Utils
{
    public class GeometryShapes
    {
        ...
        public static GeometryModel3D getCuboid(double width, double height, double depth, Vector3D position, Color? color = null)
        {
            ...
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 3, 1); // Back
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 1, 0); // Back
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 7, 1, 3); // Left
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 7, 5, 1); // Left
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 6, 5, 7); // Front
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 6, 4, 5); // Front
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 4, 6); // Left
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 0, 4); // Left
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 7, 3); // Up
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 2, 6, 7); // Up
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 0, 1, 5); // Down
            Viewport3DUtils.addTri(cuboidGeometry.TriangleIndices, 0, 5, 4); // Down
        ...
        public static GeometryModel3D getSphere(double radius, int subdivisions, Vector3D position, Color? color = null)
        {
            ...
            for (int lat = 0; lat <= subdivisions; lat++)
            {
                ...
                for (int lon = 0; lon <= subdivisions; lon++)
                {
                    ...
                    sphereGeometry.Positions.Add(new Point3D(position.X + (radius * x), position.Y + (radius * y), position.Z + (radius * z)));
            ...
            for (int lat = 0; lat < subdivisions; lat++)
            {
                for (int lon = 0; lon < subdivisions; lon++)
                {
                    ...
                    Viewport3DUtils.addTri(sphereGeometry.TriangleIndices, current, current + 1, next + 1);
                    Viewport3DUtils.addTri(sphereGeometry.TriangleIndices, current, next + 1, next);
                }
            }
        ...
        public static GeometryModel3D getSemisphere(double radius, int subdivisions, Vector3D position, Boolean? upwards = true, Color? color = null)
        {
            ...
            for (int lat = 0; lat < subdivisions / 2; lat++)
            {
                for (int lon = 0; lon < subdivisions; lon++)
                {
                    ...
                    if(upwards.Value)
                    {
                        Viewport3DUtils.addTri(semisphereGeometry.TriangleIndices, current, current + 1, next + 1);
                        Viewport3DUtils.addTri(semisphereGeometry.TriangleIndices, current, next + 1, next);
                    }
                    else
                    {
                        Viewport3DUtils.addTri(semisphereGeometry.TriangleIndices, current, next + 1, current + 1);
                        Viewport3DUtils.addTri(semisphereGeometry.TriangleIndices, current, next, next + 1);
                    }
                }
            }
        ...
        public static GeometryModel3D getCylinder(double radius, double height, int subdivisions, Vector3D position, Color? color = null)
        {
            ...
            for (int i = 0; i < subdivisions; i++)
            {
                ...
                Viewport3DUtils.addTri(cylinderGeometry.TriangleIndices, currentBottom, currentTop, nextTop);
                Viewport3DUtils.addTri(cylinderGeometry.TriangleIndices, currentBottom, nextTop, nextBottom);
            }
        ...
        public static GeometryModel3D getCapsule(double radius, double height, int subdivisions, Vector3D position, Color? color = null)
        {
            ...
            if(height > 0)
            {
                ...
                for (int i = 0; i < subdivisions; i++)
                {
                    ...
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, currentBottom, currentTop, nextTop);
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, currentBottom, nextTop, nextBottom);
                }
            }
            ...
            for (int lat = 0; lat < subdivisions / 2; lat++)
            {
                for (int lon = 0; lon < subdivisions; lon++)
                {
                    ...
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, current, current + 1, next + 1);
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, current, next + 1, next);
                }
            }
            ...
            for (int lat = 0; lat < subdivisions / 2; lat++)
            {
                for (int lon = 0; lon < subdivisions; lon++)
                {
                    ...
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, current, next + 1, current + 1);
                    Viewport3DUtils.addTri(capsuleGeometry.TriangleIndices, current, next, next + 1);
                }
            }
        ...
        public static GeometryModel3D getSquare(int size, Vector3D position, Color? color = null)
        {
            ...
            // Create a collection of triangle indices for the MeshGeometry3D.
            Viewport3DUtils.addTri(myMeshGeometry3D.TriangleIndices, 0, 1, 2);
            Viewport3DUtils.addTri(myMeshGeometry3D.TriangleIndices, 3, 4, 5);
        ...
        public static GeometryModel3D getVector(Matrix4x4 translationMatrix, double length, double width, Color? color = null)
        {
            ...
            // Create a collection of triangle indices for the MeshGeometry3D.
            Viewport3DUtils.addTri(VectorGeometry.TriangleIndices, 0, 1, 2);
            Viewport3DUtils.addTri(VectorGeometry.TriangleIndices, 2, 1, 3);
            Viewport3DUtils.addTri(VectorGeometry.TriangleIndices, 4, 5, 6);
            Viewport3DUtils.addTri(VectorGeometry.TriangleIndices, 6, 5, 7);
            ...
            Viewport3DUtils.applyTransform(vectorModel, translationMatrix);
